...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x01,                                 /* bTerminalID */
  0x01,                                 /* wTerminalType AUDIO_TERMINAL_USB_STREAMING Microphone 0x0201 */
  0x02,
  0x00,                                 /* bAssocTerminal */
  0x01,                                 /* bNrChannels */
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  /* Open IN endpoint for microphone */
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == (AUDIO_IN_EP & 0x7F)) {
	  /* Invert the buffer pointer twice, also get rid of the possible malloc artifacts in this variable */
	  haudio->in_buffer_half = !haudio->in_buffer_half;  // also serves as init to 1 or 0
	  /* Get the middle index into the variable prev */
	  uint16_t prev = (AUDIO_IN_PACKET / 2) * !haudio->in_buffer_half;
	  ADC_to_MIC();

	  USBD_LL_FlushEP (pdev, AUDIO_IN_EP); /* Flush the endpoint before making transfers */
